(() => {
  ...
  const STATES = {
    discover: { index:'01 / DISCOVER', title:'Help the customer find the right product before they need to ask.', body:'The homepage turns a large material range into a visual journey with categories and product context.', proof:['HOMEPAGE','CATALOGUE','PROJECT JOURNEYS'], outcome:'“I can actually find what I need.”', src:'./assets/desktop-light-home-section-00.png', mode:'desktop', status:'CURRENT HOMEPAGE CAPTURE' },
    search: { index:'02 / SEARCH', title:'Make product discovery resilient instead of fragile.', body:'Search and catalogue structure reduce dead ends even when customers do not know the exact product name.', proof:['SEARCH','FILTERS','CATEGORIES'], outcome:'“I’m getting closer, not lost.”', src:'./assets/desktop-light-catalogue-section-00.png', mode:'desktop', status:'CURRENT CATALOGUE CAPTURE' },
    compare: { index:'03 / COMPARE', title:'Give the decision a workspace of its own.', body:'Comparison lets customers evaluate materials before they start the conversation.', proof:['COMPARE','SHORTLIST','DECISION SUPPORT'], outcome:'“I know what I want to ask about.”', src:'./assets/desktop-light-compare.png', mode:'desktop', status:'CURRENT COMPARE CAPTURE' },
    enquire: { index:'04 / ENQUIRE', title:'Carry buying intent into the enquiry instead of resetting it.', body:'The enquiry flow preserves context so WhatsApp continues the journey instead of starting from zero.', proof:['ENQUIRY','PRODUCT CONTEXT','WHATSAPP'], outcome:'“They already know what I’m asking about.”', src:'./assets/desktop-light-enquiry-section-00.png', mode:'desktop', status:'CURRENT ENQUIRY CAPTURE' },
    theme: { index:'05 / ADAPT', title:'Keep the experience strong when the screen changes.', body:'Mobile, themes and accessibility are tested so the experience survives outside the perfect desktop screenshot.', proof:['MOBILE','THEMES','ACCESSIBILITY'], outcome:'“This feels intentional on my phone too.”', src:'./assets/mobile-light-home-section-00.png', mode:'mobile', status:'CURRENT MOBILE CAPTURE' }
  };
  ...
  const setPreview = ({ src, mode = 'desktop', label }) => {
    ...
    // State changes immediately so the UI feels responsive; the media swap gets
    // a short crossfade only. This also keeps assistive state in sync with click.
    ...
    if (deviceUrl) deviceUrl.textContent = mode === 'mobile' ? 'fakhriyarns.vercel.app / mobile' : mode === 'full' ? 'fakhriyarns.vercel.app / full page' : 'fakhriyarns.vercel.app';
