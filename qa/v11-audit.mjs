...
for(const [name,width,height] of viewports){
  ...
  if(await page.locator('.v11-hero-ghost').count()!==3)fail(name,'hero refraction layers missing');
  ...
  // Exercise pointer and scroll velocity so the bending engine actually updates CSS variables.
  await page.mouse.move(width*.2,height*.35); await page.mouse.move(width*.82,height*.65,{steps:8});
  await page.evaluate(()=>scrollTo(0,Math.round(innerHeight*.72))); await page.waitForTimeout(260);
  ...
  if(name!=='mobile' && Number(vars.fold)<=0)fail(name,'hero fold did not respond to scroll',{vars});
  ...
  await page.locator('#manifesto').scrollIntoViewIfNeeded(); await page.waitForTimeout(550);
  ...
  await page.locator('#plans').scrollIntoViewIfNeeded(); await page.waitForTimeout(500);
  const stageTransform=await page.locator('.pricing-stage').evaluate(el=>getComputedStyle(el).transform);
  if(!stageTransform||stageTransform==='none')fail(name,'pricing depth transform missing',{stageTransform});
  const once=page.locator('.pricing-mode [data-mode="once"]'); await once.click(); await page.waitForTimeout(180);
  ...
  await business.click(); await page.waitForTimeout(130);
  ...
  await page.locator('#work').scrollIntoViewIfNeeded(); await page.waitForTimeout(650);
  ...
  await page.locator('#audit').scrollIntoViewIfNeeded(); await page.waitForTimeout(350);
  ...
  await page.locator('#transform').scrollIntoViewIfNeeded(); await page.waitForTimeout(300);
