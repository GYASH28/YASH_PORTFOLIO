...
async function dismissIntro(page){
  ...
  if(await skip.count()) try{if(await skip.isVisible({timeout:700})) await skip.click({timeout:1500});}catch{}
...
async function basicAudit(page,scope){
  ...
  const imgs=await page.locator('img').evaluateAll(imgs=>imgs.map(img=>({src:img.getAttribute('src'),complete:img.complete,naturalWidth:img.naturalWidth,naturalHeight:img.naturalHeight}))); 
  for(const img of imgs) if(!img.complete||img.naturalWidth===0) fail(scope,'broken image',{src:img.src});

  const duplicateIds=await page.evaluate(()=>{const m=new Map();for(const el of document.querySelectorAll('[id]'))m.set(el.id,(m.get(el.id)||0)+1);return[...m.entries()].filter(([,n])=>n>1)});
  if(duplicateIds.length) fail(scope,'duplicate ids',{duplicateIds});

  const emptyButtons=await page.locator('button').evaluateAll(btns=>btns.filter(b=>!((b.innerText||b.getAttribute('aria-label')||'').trim())).map(b=>b.outerHTML.slice(0,180)));
  if(emptyButtons.length) fail(scope,'buttons without accessible label',{emptyButtons});

  const tinyTargets=await page.locator('a:visible,button:visible,input:visible').evaluateAll(els=>els.map(el=>{const r=el.getBoundingClientRect();return{text:(el.innerText||el.getAttribute('aria-label')||'').trim().slice(0,60),w:r.width,h:r.height}}).filter(x=>x.w>0&&x.h>0&&(x.w<32||x.h<32)));
...
async function homepageInteractions(page,scope){
  ...
  if(await canvas.count()){
    ...
    if(!box||box.width<100||box.height<100) fail(scope,'hero canvas has invalid size',{box});
  }

  await go(page,'#plans');
  const once=page.locator('.mode[data-mode="once"]');
  if(await once.count()){
    await once.click(); await page.waitForTimeout(180);
    const count=await page.locator('.plan-tab[data-mode="once"]:visible').count();
    if(count!==3) fail(scope,'pay-once mode should show 3 plans',{count});
  }
  const monthly=page.locator('.mode[data-mode="monthly"]'); if(await monthly.count()) await monthly.click();

  await go(page,'#work');
  const caseBtns=page.locator('.case-progress button');
  const caseCount=await caseBtns.count();
  if(caseCount!==4) fail(scope,'FakhriMart case study should have 4 stages',{count:caseCount});
  else for(let i=0;i<4;i++){
    await caseBtns.nth(i).click(); await page.waitForTimeout(150);
  ...
  if(await testItem.count()){
    ...
    if(await testItem.getAttribute('aria-pressed')!=='true') fail(scope,'website-test item did not toggle');
  }
...
async function experienceInteractions(page,scope){
  ...
  const dc=await devices.count();
  if(dc!==4) fail(scope,'Experience page should expose 4 device modes',{count:dc});
  else for(let i=0;i<3;i++){
    await devices.nth(i).click(); await page.waitForTimeout(120);
    const got=await page.locator('#deviceStage').getAttribute('data-device');
    const expected=['desktop','mobile','full'][i];
    if(got!==expected) fail(scope,'device mode did not update',{expected,got});
  }

  const features=page.locator('.feature-rail .feature');
  const fc=await features.count();
  if(fc!==5) fail(scope,'Experience page should expose 5 feature stories',{count:fc});
  else{
    const seen=new Set();
    for(let i=0;i<5;i++){
      await features.nth(i).click(); await page.waitForTimeout(100);
      const title=(await page.locator('#storyTitle').textContent())?.trim()||''; seen.add(title);
  ...
  await go(page,'#captures');
  const gallery=page.locator('.capture-card');
  const gc=await gallery.count(); if(gc!==4) fail(scope,'capture gallery should expose 4 states',{count:gc});
  if(gc){
    await gallery.first().click(); await page.waitForTimeout(100);
    const dlg=page.locator('#captureDialog');
    if(!(await dlg.evaluate(el=>el.open))) fail(scope,'capture lightbox did not open');
    const close=page.locator('#captureDialogClose'); if(await close.count()) await close.click();
  }
}
...
async function auditRoute(page,vp,route){
  ...
  try{
    ...
    await page.waitForLoadState('networkidle',{timeout:12000}).catch(()=>warn(scope,'network did not become idle within 12s'));
    ...
    fail(scope,'interaction audit threw',{error:String(err)});
  }finally{
    if(consoleErrors.length) fail(scope,'console errors',{consoleErrors});
    if(pageErrors.length) fail(scope,'page errors',{pageErrors});
    try{await page.screenshot({path:path.join(outDir,`${vp.name}-${route.replace('.html','')}.png`),fullPage:true,animations:'disabled',timeout:30000});}catch(err){warn(scope,'screenshot failed',{error:String(err)});}
    try{metrics.push({scope,url:page.url(),title:await page.title(),scrollHeight:await page.evaluate(()=>document.documentElement.scrollHeight)});}catch{}
...
async function run(){
  ...
  try{
    ...
    for(const vp of viewports){
      ...
      await auditRoute(page,vp,'index.html');
      await auditRoute(page,vp,'experience.html');
      await context.close();
    }
    const context=await browser.newContext({viewport:{width:390,height:844},reducedMotion:'reduce'});
    const page=await context.newPage();
    try{await page.goto(`${base}/index.html`,{waitUntil:'domcontentloaded',timeout:45000});await page.waitForTimeout(350);await dismissIntro(page);await basicAudit(page,'reduced-motion/index');}
    catch(err){fail('reduced-motion/index','smoke test threw',{error:String(err)});}
    try{await page.screenshot({path:path.join(outDir,'reduced-motion-mobile.png'),fullPage:true,animations:'disabled'});}catch{}
    await context.close();
  }finally{
  ...
  if(failures.length){console.error(`UI audit failed with ${failures.length} failure(s).`);process.exit(1);}
