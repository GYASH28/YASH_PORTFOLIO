...
async function waitVisibleImages(page,root='body'){
  const imgs=page.locator(`${root} img:visible`);const n=await imgs.count();
  for(let i=0;i<n;i++){
    const img=imgs.nth(i);
    await img.evaluate(el=>{try{el.loading='eager'}catch{}}).catch(()=>{});
  }
  await page.waitForTimeout(120);
}
async function auditLoadedImages(page,scope){
  const bad=await page.locator('img:visible').evaluateAll(imgs=>imgs.filter(img=>img.getBoundingClientRect().width>0&&img.getBoundingClientRect().height>0&&(!img.complete||img.naturalWidth===0)).map(img=>({src:img.getAttribute('src'),loading:img.loading})));
  bad.forEach(img=>fail(scope,'visible image failed to load',img));
}
...
async function basicAudit(page,scope){
  ...
  await auditLoadedImages(page,scope);
...
async function experienceInteractions(page,scope){
  await go(page,'#lab');await waitVisibleImages(page,'#lab');await auditLoadedImages(page,scope);
