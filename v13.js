(()=>{
  ...
  const tick=()=>{
    ...
    if(hero&&!reduce){
      ...
      hero.style.setProperty('--heroExit',p.toFixed(3));
      const bg=hero.querySelector('.hero-bg'),front=hero.querySelector('.hero-front'),name=hero.querySelector('.hero-name');
      if(bg)bg.style.marginTop=`${p*34}px`;
      if(front)front.style.marginTop=`${p*58}px`;
      if(name)name.style.marginTop=`${p*-40}px`;
    }
  ...
  function setMode(mode){
    modeBtns.forEach(b=>{const on=b.dataset.planMode===mode;b.classList.toggle('active',on);b.setAttribute('aria-selected',String(on))});
    planCards.forEach(c=>{c.hidden=c.dataset.planModel!==mode});
  }
  modeBtns.forEach(b=>b.addEventListener('click',()=>setMode(b.dataset.planMode)));
  if(modeBtns.length)setMode('managed');

  // Enquiry dialog.
  const dialog=document.querySelector('#enquiryDialog');
  const dialogPlan=document.querySelector('#dialogPlan');
  document.querySelectorAll('.js-enquire').forEach(btn=>btn.addEventListener('click',()=>{
    if(dialogPlan)dialogPlan.textContent=btn.dataset.plan||'Website project';
    if(dialog?.showModal)dialog.showModal();
  }));
  document.querySelectorAll('[data-close-dialog]').forEach(btn=>btn.addEventListener('click',()=>dialog?.close()));
  dialog?.addEventListener('click',e=>{if(e.target===dialog)dialog.close()});

  // FakhriMart customer journey on work page.
  const workImg=document.querySelector('#journeyImage');
  const workTitle=document.querySelector('#journeyTitle');
  const workBody=document.querySelector('#journeyBody');
  const workOutcome=document.querySelector('#journeyOutcome');
  const workStep=document.querySelector('#journeyStep');
  const journey=[
  ...
  function setJourney(i){
    const d=journey[i];if(!d||!workImg)return;
    journeyBtns.forEach((b,n)=>b.classList.toggle('active',n===i));
    workImg.style.opacity='.35';
    const next=new Image();next.onload=()=>{workImg.src=d.img;workImg.alt=`FakhriMart ${d.step.toLowerCase()} screen`;workImg.style.opacity='1'};next.src=d.img;
    if(workTitle)workTitle.textContent=d.title;if(workBody)workBody.textContent=d.body;if(workOutcome)workOutcome.textContent=d.outcome;if(workStep)workStep.textContent=d.step;
  }
  journeyBtns.forEach((b,i)=>b.addEventListener('click',()=>setJourney(i)));

  // Anchor navigation closes focus gap under fixed header.
