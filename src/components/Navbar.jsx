      ...
      >
        <nav
          className={`relative mx-auto max-w-6xl rounded-2xl flex items-center justify-between px-5 sm:px-6 h-16 sm:h-[72px] transition-all duration-300 pointer-events-auto ${
            scrolled
              ? "bg-[#05060A]/80 backdrop-blur-[20px] [-webkit-backdrop-filter:blur(20px)] border border-white/[0.08] shadow-[0_10px_40px_rgba(0,0,0,0.5)]"
        ...
        >
          ...
          >
            <span className="relative inline-flex items-center justify-center w-10 h-10 rounded-xl bg-white/[0.03] border border-white/10 overflow-hidden group-hover:scale-105 transition-transform">
              ...
              <span className="relative text-sm text-[var(--text-1)]" style={{ letterSpacing: '2px', fontFamily: '"Bricolage Grotesque", sans-serif', fontWeight: 700 }}>YG</span>
            </span>
          ...
          <div className="hidden lg:flex items-center gap-2 relative">
            {LINKS.map(l => (
              <button
                ...
                className={`relative px-4 py-2 text-[12px] font-mono tracking-widest uppercase transition-all duration-300 z-10 ${
                  active === l.id ? "text-white font-bold" : "text-white/55 hover:text-white"
                }`}
              >
                ...
                {active === l.id && (
                  <motion.span
                    ...
                    className="absolute inset-0 rounded-xl bg-white/[0.03] border border-[#00F5FF]/15 -z-10"
                    style={{ 
                      boxShadow: "0 0 20px rgba(0, 245, 255, 0.04), inset 0 1px 0 rgba(255,255,255,0.05)",
                    }}
