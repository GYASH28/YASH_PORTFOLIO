(() => {
  ...
  const dismissBoot = () => {
    ...
    if (hasGSAP && !reduceMotion) gsap.to(boot, { yPercent: -100, duration: .72, ease: 'power4.inOut', onComplete: () => boot.remove() });
    else boot.remove();
  };
  if (boot) {
    if (reduceMotion || session.get('ykg-v3-intro-seen')) boot.remove();
    else {
      ...
      if (hasGSAP) {
        ...
        tl.to('.boot-line span', { scaleX: 1, duration: 1.55, ease: 'none' }, 0)
      ...
      } else setTimeout(dismissBoot, 1700);
    }
  }
  ...
  const decisionData = [
    { word: 'TRUST', prompt: '“Do these people look credible?”', title: 'Make the business feel worth choosing.', body: 'Art direction, type, hierarchy and real proof should make the quality of the business obvious before the visitor reads every word.' },
    { word: 'FIT', prompt: '“Is this actually for someone like me?”', title: 'Remove the effort from understanding the offer.', body: 'The page should surface the right products, services, pricing cues and context in the order a real customer needs them.' },
    { word: 'ACTION', prompt: '“Fine. What do I do now?”', title: 'End every useful thought with a next step.', body: 'WhatsApp, enquiry, booking, directions or checkout — the website should hand people to the action your business can actually fulfil.' }
  ];
  ...
  const renderDecision = (index, progressWithin = 1) => {
    ...
    if (dMeter) dMeter.style.transform = `scaleX(${Math.min(1, (index + Math.max(.12, progressWithin)) / 3)})`;
  };
  renderDecision(0, .15);

  if (hasGSAP && hasST && !reduceMotion) {
    ScrollTrigger.create({ trigger: '.decision', start: 'top top', end: 'bottom bottom', scrub: .45, onUpdate(self) {
    ...
    const onScrollDecision = () => {
      ...
      const scaled = p * 3;
      renderDecision(Math.floor(scaled), scaled - Math.floor(scaled));
    };
  ...
  if (hasGSAP && hasST && !reduceMotion) {
    caseNotes.forEach((note, i) => ScrollTrigger.create({ trigger: note, start: 'top 62%', end: 'bottom 38%', onToggle(self) {
  ...
  form?.addEventListener('submit', (e) => {
    ...
    const lines = [`Hi Yash, I'm interested in the ${fd.get('plan')}.`, `Name: ${fd.get('name')}`, `Business: ${fd.get('business')}`, `Phone: ${fd.get('phone')}`, fd.get('website') ? `Current website: ${fd.get('website')}` : '', `Goal: ${fd.get('goal')}`].filter(Boolean);
  ...
  if (hasGSAP && hasST && !reduceMotion) {
    gsap.utils.toArray('.work-copy h2,.work-copy p,.offer-head>* ,.plans-head>* ,.objections-title>*').forEach((el) => gsap.from(el, { y: 36, autoAlpha: 0, duration: .8, ease: 'power3.out', scrollTrigger: { trigger: el, start: 'top 88%', once: true } }));
